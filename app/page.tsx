...
export default function Home() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      savePlayerId(result.gameCode, result.playerId)
  ...
  async function handleJoin() {
    ...
    setError('')
    try {
      const result = await joinGame(code.toUpperCase(), name)
      savePlayerId(result.gameCode, result.playerId)
      router.push(`/game/${result.gameCode}`)
    } catch (err: any) {
      setError(err.message)
      ...
      <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl w-96 border border-gray-700">
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 py-2 rounded font-bold text-sm transition ${mode === 'coop' ? 'bg-green-600' : 'bg-gray-700 text-gray-400'}`}
          >
            🤝 Coop
          </button>
          <button
            onClick={() => setMode('versus')}
            className={`flex-1 py-2 rounded font-bold text-sm transition ${mode === 'versus' ? 'bg-red-600' : 'bg-gray-700 text-gray-400'}`}
          >
            ⚔️ Versus
          </button>
        </div>
